...
const Navbar = () => {
    ...
    return (
        <div className="w-full">
            <div className={`mx-auto flex flex-col items-center border-t-8 border-white border-l-8 border-r-8  ${location.pathname === '/home' ? 'bg-purple-600' : 'bg-white text-black'}`}>
                {/* Navbar */}
                <nav className={`w-4/5 flex justify-between items-center py-4 mt-4 ${location.pathname === '/home' ? 'bg-purple-600 text-white' : 'bg-white text-black'}`}>
                    ...
                    <div className="hidden md:flex space-x-4">
                        <button className={`rounded-full p-2 border-2 ${iconColor} ${iconColor === 'text-purple-600' ? 'text-white' : 'border-white'}`}>
                ...
                {menuOpen && (
                    <div className="fixed inset-0 bg-purple-500 bg-opacity-90 flex flex-col justify-between text-white w-3/4 md:hidden z-10">
                        ...
                        <div className="flex space-x-4 p-6 justify-center">
                            <button className={`rounded-full p-2 border-2 ${mobileIconColor} ${mobileBorderColor}`}>
                                <MdOutlineShoppingCart />
                            </button>
                            <button className={`rounded-full p-2 border-2 ${mobileIconColor} ${mobileBorderColor}`}>
                                <FaRegHeart />
